...
| 事業者 | 全国件数 | 一括取得 | 1巡コスト(4秒間隔) |
|---|---|---|---|
| NPC | 約1,700 | ✅ bbox API 1発 | 数秒 |
| リパーク | 約16,000 | ❌ 1物件=1ページ | **約18時間** |

リパークは一括APIが無く、全件を1日数回取得するのは「節度ある収集」と両立しない。
→ **ローリング巡回**（毎回 `reparkRollingPerRun` 件ずつ、最終取得が古い順に取得）で
数日かけて全国を1巡する設計。巡回状態は `data/repark-crawl-state.json` に保持。
