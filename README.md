...
3社は料金の持ち方が異なる（分刻みが 15/20/30/60 分、最大料金の条件が自由文）ため、
`src/normalize.js` で**共通形に正規化**して横比較できるようにしている。各レコードに `fee` を付与：

- `fee.yph` … 時間単価を **円/時** に正規化（代表＝平日/全日 昼14時の単価）
- `fee.max24h` … **24時間・当日最大料金**（比較の主指標。時間制限maxしかない物件は null）
- `fee.maxNight` … 夜間最大（あれば）
- `fee.unit[]` / `fee.max[]` … 曜日区分(day)・時間帯・種別(d24h/night/daytime/duration)で構造化

```bash
npm run export   # 最新スナップショットを data/parking-latest.csv に出力（Excel用・BOM付き）
```

CSV 列: 事業者 / 物件名 / 都道府県 / 住所 / 収容台数 / 円per時 / 最大料金_24時間 / 最大料金_夜間 / 緯度 / 経度 / 取得日時 / URL
...
`dashboard.html` は**自己完結（外部CDN不使用・データ埋め込み）**。ブラウザで直接開くだけで見られる。内容:

- KPI（総物件数・事業者別・累計変動数）
- 事業者別の料金水準（円/時・最大料金の中央値）
- 時間単価・最大料金の分布ヒストグラム（事業者で色分け）
- 都道府県別 物件数（上位15）
- 地理分布（経度×緯度の散布図。NPC/リパークは座標あり、タイムズは座標なし）
- 最近の料金変動（収集が一巡すると差分が出る）
- 物件一覧（名称・都道府県で検索）
