...
- 対応事業者と取得単位:
  ...
  - **ナビパーク（スターツアメニティー / navipark1.com）** — 静的HTML（`table05`）。エリア階層（県→市区→物件）を辿って列挙・ローリング巡回。別ワークフロー（`crawl-navipark.yml`）・`prices-navipark.jsonl`
  - **エコロパーク（エコロシティ / service.ecolocity.co.jp）** — 静的HTML（基本情報テーブル）。約195エリアを辿って列挙・ローリング巡回。別ワークフロー（`crawl-ecolo.yml`）・`prices-ecolo.jsonl`
  - **ザ・パーク（第一興商 / dkparking.com）** — 単一JSON（`data/search.json`）で**全国 約3,367件を1リクエスト取得**（座標フィールドが逆なので補正）。`crawl-others.yml`（相乗り）・`prices-others.jsonl`
